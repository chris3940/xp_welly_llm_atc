...
void on_ptt_pressed() {
    if (state_ != PTTState::IDLE) {
        ...
        std::snprintf(buf, sizeof(buf),
                      "[xp_wellys_atc] PTT blocked, state=%d\n",
...
void on_ptt_released() {
    if (state_ != PTTState::RECORDING) return;
    ...
    char buf[256];
    std::snprintf(buf, sizeof(buf),
                  "[xp_wellys_atc] PTT released — duration=%.2fs, samples=%zu, wav=%zu bytes\n",
...
std::string ptt_state_label() {
    switch (state_) {
        ...
        case PTTState::RECORDING:  return "\xE2\x97\x8F REC";        // ● REC
        case PTTState::PROCESSING: return "\xE2\x9F\xB3 PROCESSING"; // ⟳ PROCESSING
        case PTTState::PLAYING:    return "\xE2\x96\xB6 PLAYING";    // ▶ PLAYING
    }
    return "UNKNOWN";
}
