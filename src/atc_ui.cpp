...
static const char* voice_names[] = {"alloy", "echo", "fable", "onyx", "nova", "shimmer"};
...
static void draw_status_tab() {
    ...
    float active_freq = (ctx.active_com == 1) ? ctx.com1_freq_mhz : ctx.com2_freq_mhz;
    ...
    ImGui::Text("GS: %.0f kts   IAS: %.0f kts",
                ctx.groundspeed_kts, ctx.indicated_airspeed_kts);
    ImGui::Text("VS: %.0f fpm   HDG: %.0f",
                ctx.vertical_speed_fpm, ctx.heading_true);
...
static void draw_settings_tab() {
    ...
    if (!buffers_initialized) {
        ...
        for (int i = 0; i < voice_count; ++i) {
            if (voice == voice_names[i]) { voice_selection = i; break; }
        }
    ...
    if (key_just_saved && save_feedback_timer > 0.0f) {
        ...
        if (save_feedback_timer <= 0.0f) key_just_saved = false;
    ...
    if (ptt_vk < 0) {
        ImGui::TextDisabled("PTT: not configured (set ptt_key_vk in settings.json)");
    } else {
