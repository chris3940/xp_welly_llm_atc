...
void init() {
    ...
    if (!component) {
        XPLMDebugString("[xp_wellys_atc] Error: no HALOutput audio component found\n");
        return;
    }
    ...
    initialized_ = true;
    XPLMDebugString("[xp_wellys_atc] Audio recorder initialized (16kHz mono 16-bit)\n");
}
...
void start_recording() {
    if (!initialized_ || !audio_unit_) {
        XPLMDebugString("[xp_wellys_atc] Warning: audio recorder not initialized\n");
        return;
    }
...
void init() {
    XPLMDebugString("[xp_wellys_atc] Warning: audio recorder not supported on this platform\n");
}
void stop() {}
void start_recording() {}
void stop_recording() {}
...
std::vector<uint8_t> encode_wav() {
    ...
    auto write_str = [&](const char* s) {
        while (*s) wav.push_back(static_cast<uint8_t>(*s++));
    };
    ...
    write_u32(kSampleRate * kNumChannels * sizeof(int16_t));  // byte rate
