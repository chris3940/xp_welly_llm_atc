...
void init() {
    ...
    if (in.good()) {
        try {
            ...
            XPLMDebugString("[xp_wellys_atc] Warning: failed to parse settings.json, using defaults\n");
            cfg = default_config();
        }
    } else {
        cfg = default_config();
    ...
    if (cfg.value("api_key_saved", false)) {
        ...
        if (cached_api_key.empty()) {
            XPLMDebugString("[xp_wellys_atc] Warning: api_key_saved flag set but Keychain load failed\n");
...
bool save_api_key(const std::string& key) {
    ...
    XPLMDebugString("[xp_wellys_atc] Error: failed to save API key to Keychain\n");
    return false;
}
...
std::string whisper_model()       { return cfg.value("whisper_model", std::string("whisper-1")); }
std::string gpt_model()           { return cfg.value("gpt_model", std::string("gpt-4o-mini")); }
bool        gpt_fallback_enabled(){ return cfg.value("gpt_fallback_enabled", true); }
std::string pilot_callsign()      { return cfg.value("pilot_callsign", std::string("November One Two Three Alpha Bravo")); }
