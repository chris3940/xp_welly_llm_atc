...
std::string read_system_text() {
    ...
    // One-time probe: warn in Log.txt if the required tool is absent.
    // popen() itself succeeds as long as /bin/sh starts, so an empty
    // result is otherwise indistinguishable from "clipboard is empty".
